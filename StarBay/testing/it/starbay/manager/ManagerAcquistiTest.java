...
public class ManagerAcquistiTest {
	...
	{	
		...
		result = statement.executeQuery("SELECT idDettaglioOrdine, coordinate FROM INCLUDE_STELLE WHERE coordinate='220,98 +34,67 11,13'");
		Ordine newO = new Ordine();
		newO.setIdDettaglioOrdine(result.getInt(1));
		newO.setIdProdotto(result.getString(2));
		assertEquals("220,98 +34,67 11,13", newO.getIdProdotto());
		assertEquals(""+o.getIdDettaglioOrdine(), ""+newO.getIdDettaglioOrdine());
		statement.executeUpdate("DELETE FROM INCLUDE_STELLE WHERE coordinate='220,98 +34,67 11,13'");
	}

	@Test
	public void testGetCoordinate() throws ClassNotFoundException, SQLException 
	{
		assertEquals("220,98 +34,67 11,13", ma.getCoordinate("Melissa"));
	}
